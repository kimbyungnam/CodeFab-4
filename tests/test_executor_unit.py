...
def run_and_capture_prints(mocker, statements) -> list:
    print_mock = mocker.patch("builtins.print")
    ExecutorUnit().execute(statements)
    return [call.args[0] for call in print_mock.call_args_list]
...
def test_이항_연산_결과를_출력한다(mocker, left, operator, right, expected):
    # 출력 <left> <operator> <right>;
    statement = PrintStmt(
        make_binary(left=Literal(left), operator_lexeme=operator, right=Literal(right))
    )

    assert run_and_capture_prints(mocker, [statement]) == [expected]
...
def test_복합_표현식_트리를_우선순위와_결합_법칙에_따라_평가한다(
    mocker, build_expression, expected
):
...
def test_리터럴_값을_형식에_맞게_출력한다(mocker, value, expected):
    # 출력 <value>;
...
def test_변수를_선언하고_재할당하면_최신_값을_사용한다(mocker):
    ...
    assert run_and_capture_prints(mocker, statements) == ["30", "15"]


def test_블록_스코프는_바깥_변수를_가리고_블록이_끝나면_복원된다(mocker):
    ...
    assert run_and_capture_prints(mocker, statements) == ["안쪽", "전역"]


def test_블록_내부의_대입은_바깥_스코프_변수를_수정한다(mocker):
    ...
    assert run_and_capture_prints(mocker, statements) == ["1"]


def test_중첩된_블록에서_바깥_스코프_변수를_참조할_수_있다(mocker):
    ...
    assert run_and_capture_prints(mocker, statements) == ["AB"]
...
def test_만약_조건이_참이면_then_branch를_실행한다(mocker):
    ...
    assert run_and_capture_prints(mocker, [var_declare, if_stmt]) == ["5"]
...
def test_만약_조건이_거짓이고_아니면이_없으면_아무것도_출력하지_않는다(mocker):
    ...
    assert run_and_capture_prints(mocker, [if_stmt]) == []
...
def test_만약_조건이_거짓이면_아니면_branch를_실행한다(mocker):
    ...
    assert run_and_capture_prints(mocker, [if_stmt]) == ["kfc"]


def test_아니면은_블록으로_감싼_가장_가까운_만약에_결합된다(mocker):
    ...
    assert run_and_capture_prints(mocker, [outer_if]) == ["bbq"]
...
def test_만약_조건을_비교_연산자로_평가한다(
    ...
    assert run_and_capture_prints(mocker, [if_stmt]) == [expected]
...
def test_반복문이_조건을_만족하는_동안_반복해서_출력한다(mocker):
    ...
    assert run_and_capture_prints(mocker, [for_stmt]) == ["0", "1", "2"]
...
def test_반복문_조건이_처음부터_거짓이면_한번도_실행되지_않는다(mocker):
    ...
    assert run_and_capture_prints(mocker, [for_stmt]) == []
