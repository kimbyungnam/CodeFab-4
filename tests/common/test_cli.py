...
from codefab.common.cli import FileRunner, main
...
def test_파일을_읽어_interpreter에_source로_전달한다(mocker, tmp_path):
    ...
    runner = FileRunner(interpreter=interpreter, output=mocker.Mock())
...
def test_output_라인이_순서대로_output_콜백에_전달된다(mocker, tmp_path):
    ...
    calls = []
    runner = FileRunner(interpreter=interpreter, output=calls.append)
...
def test_main이_run_경로_인자를_file_runner_run_file에_전달한다(mocker):
    run_file = mocker.patch.object(FileRunner, "run_file", return_value=0)
...
def test_main에_인자가_없으면_repl_모드로_진입한다(mocker):
    repl_main = mocker.patch("codefab.common.cli.repl_main", return_value=0)
