...
def test_괄호로_감싼_상수도_접힌다(mocker):
    ...
    stmt = mocker.Mock(
        spec=ExpressionStmt, expression=mocker.Mock(spec=Grouping, expression=inner)
    )
...
def test_단항_마이너스_상수는_접힌다(mocker):
    right = mocker.Mock(spec=Literal, value=3.0)
    expr = mocker.Mock(
        spec=Unary, operator=make_operator_token(TokenType.MINUS, "-"), right=right
    )
    stmt = mocker.Mock(spec=ExpressionStmt, expression=expr)
...
def test_단항_bang_상수는_접힌다(mocker):
    ...
    expr = mocker.Mock(
        spec=Unary, operator=make_operator_token(TokenType.BANG, "!"), right=right
    )
    stmt = mocker.Mock(spec=ExpressionStmt, expression=expr)
...
def test_변수에_대한_단항연산은_접히지_않는다(mocker):
    variable = mocker.Mock(spec=Variable)
    expr = mocker.Mock(
        spec=Unary, operator=make_operator_token(TokenType.MINUS, "-"), right=variable
    )
    stmt = mocker.Mock(spec=ExpressionStmt, expression=expr)
...
def test_숫자_연산자들이_리터럴로_접힌다(
    mocker, op_type, op_lexeme, left, right, expected
):
    expr = mocker.Mock(
        spec=Binary,
...
def test_array_리터럴의_size와_인덱스_표현식도_접힌다(mocker):
    ...
    index_get = mocker.Mock(
        spec=IndexGet, target=array_literal, index=index_expr, line=1
    )
